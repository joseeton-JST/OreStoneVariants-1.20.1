...
import static io.github.joseetoon.genlib.exception.Exceptions.missingElement;
...
public class SafeRegistry<K, V> implements Map<K, V>, Iterable<V> {
     ...
     */
    public static <K, V> SafeRegistry<K, V> of(final Supplier<Map<K, V>> event) {
     ...
     * @return An immutable registry which cannot be refreshed.
     */
    public static <V> SafeRegistry<Integer, V> enumerated(final Supplier<Collection<V>> event) {
     ...
     * @return An immutable registry which cannot be refreshed.
     */
    public static <V> SafeRegistry<Integer, V> enumerated(final Collection<V> data) {
        return new SafeRegistry<>(new Lazy<>(enumerate(data)), DEFAULT_ERROR::apply);
    }
     ...
     * @return An immutable registry which cannot be refreshed.
     */
    public static <V> SafeRegistry<Integer, V> enumerated(final Supplier<Collection<V>> event, final ErrorFunction<Integer> err) {
     ...
     * @return A new registry with the updated error info.
     */
    public SafeRegistry<K, V> respondsWith(final Function<K, String> msg) {
        return new SafeRegistry<>(this.map, k -> missingElement(msg.apply(k)));
    }
     ...
     */
    public SafeRegistry<K, V> canBeReset(final boolean resettable) {
        if (resettable == this.map.isResettable()) {
            return this;
        }
        return new SafeRegistry<>(this.map.asResettable(resettable), this.err);
    }
