...
import static io.github.joseetoon.genlib.io.FileIO.listFiles;
...
public class PathUtils {
    ...
    public static ResourceLocation getResourceLocation(final String path) {
        ...
        final String result = path
            ...
            .replaceAll("^[/\\\\]", "");
        return ResourceLocation.fromNamespaceAndPath(namespace, result);
    }
    ...
    public static ResourceLocation fromRawPath(final String path) {
        ...
        if (content < 0) return ResourceLocation.parse(path);
        final String fromMod = path.substring(content + 1);

        final int mod = fromMod.indexOf("/");
        if (mod < 0) return ResourceLocation.parse(path);
        final String modName = fromMod.substring(0, mod);
        final String fullPath = fromMod.substring(mod + 1);

        final int key = fullPath.indexOf("/");
        if (key < 0) return ResourceLocation.fromNamespaceAndPath(modName, fullPath + 1);

        return ResourceLocation.fromNamespaceAndPath(modName, noExtension(fullPath.substring(key + 1)));
    }
