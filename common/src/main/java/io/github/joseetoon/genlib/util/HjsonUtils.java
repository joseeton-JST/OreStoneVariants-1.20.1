...
import io.github.joseetoon.genlib.data.*;
import io.github.joseetoon.genlib.data.JsonType;
import io.github.joseetoon.genlib.exception.JsonFormatException;
import io.github.joseetoon.genlib.exception.UnreachableException;
...
import static io.github.joseetoon.genlib.exception.Exceptions.jsonFormatEx;
...
import static io.github.joseetoon.genlib.exception.Exceptions.unreachable;
import static io.github.joseetoon.genlib.util.McUtils.getBiome;
import static io.github.joseetoon.genlib.util.McUtils.parseBlockState;
import static io.github.joseetoon.genlib.util.Shorthand.f;
import static io.github.joseetoon.genlib.util.Shorthand.full;
import static io.github.joseetoon.genlib.util.Shorthand.assertEnumConstant;
...
public class HjsonUtils {
    ...
    public static <T> T readThrowing(final Codec<T> codec, final JsonValue value) {
        return codec.parse(HjsonOps.INSTANCE, value)
            .resultOrPartial(partial -> {
    ...
    public static Optional<ResourceLocation> getId(final JsonObject json, final String field) {
        return getString(json, field).map(ResourceLocation::parse);
    }
    ...
    public static List<ResourceLocation> toIds(final JsonArray array) {
        ...
        for (final JsonValue value : array) {
            ids.add(ResourceLocation.parse(value.asString()));
        }
        return ids;
    }
    ...
    public static List<Biome> toBiomes(final JsonValue json) {
        ...
        if (json.isObject()) {
            ...
            // BiomeCategory was removed in 1.20 — type-based filtering is no longer supported.
        } else {
