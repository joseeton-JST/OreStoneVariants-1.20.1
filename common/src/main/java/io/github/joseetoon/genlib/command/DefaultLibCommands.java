...
import io.github.joseetoon.genlib.client.gui.SimpleTextPage;
...
import io.github.joseetoon.genlib.util.JsonCombiner;
...
import static io.github.joseetoon.genlib.command.CommandSuggestions.CURRENT_JSON;
import static io.github.joseetoon.genlib.command.CommandUtils.arg;
import static io.github.joseetoon.genlib.command.CommandUtils.fileArg;
import static io.github.joseetoon.genlib.command.CommandUtils.greedyArg;
import static io.github.joseetoon.genlib.command.CommandUtils.jsonFileArg;
import static io.github.joseetoon.genlib.command.CommandUtils.jsonPathArg;
...
public class DefaultLibCommands {
    ...
    public static LibCommandBuilder createCaveTestAlias(final ModDescriptor mod) {
        return LibCommandBuilder.named("cave")
            .append("Root alias for cave-viewing utilities.")
            .type(CommandType.GLOBAL)
            .generate((builder, utl) -> builder
                .then(literal("test")
                    .executes(utl.wrap(DefaultLibCommands::test))));
    }
    ...
    private static void display(final CommandContextWrapper wrapper) {
        ...
        if (McUtils.isClientSide() && numLines >= LibConfig.displayLength()) {
            loadTextPage(wrapper, detailsComponent, headerComponent);
            return;
        }
    ...
    private static MutableComponent generateUndo(final String input, final String from, final String to) {
        ...
        return (MutableComponent) Component.literal("[UNDO]").setStyle(UNDO_STYLE.withClickEvent(undo));
    }
    ...
    private static void test(final CommandContextWrapper wrapper) {
        ...
        if (player instanceof net.minecraft.server.level.ServerPlayer) {
            final net.minecraft.server.level.ServerPlayer sp = (net.minecraft.server.level.ServerPlayer) player;
            sp.setGameMode(GameType.SPECTATOR);

            sp.addEffect(new MobEffectInstance(MobEffects.NIGHT_VISION, 999999999, 1, true, false));
        }
    }
    ...
    private static void ch(final CommandContextWrapper wrapper, final boolean max) {
        wrapper.sendError("Chat height command is not supported in this version.");
    }

    private static void cw(final CommandContextWrapper wrapper, final boolean max) {
        wrapper.sendError("Chat width command is not supported in this version.");
    }
