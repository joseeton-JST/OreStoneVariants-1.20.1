...
import io.github.joseetoon.genlib.client.ClientErrorBroadcaster;
...
import io.github.joseetoon.genlib.exception.ModLoadException;
...
public class LibErrorContext {
    ...
    private static void broadcastErrors() {
        ClientErrorBroadcaster.broadcastErrors();
    }
    ...
    @ApiStatus.Internal
    @Environment(EnvType.CLIENT)
    public static void logClientErrors() {
        COMMON_ERRORS.forEach((mod, errors) -> errors.forEach(error -> log.error(
            "Client error menu entry [{}] title='{}' message='{}'",
            mod.getModId(),
            error.getTitleMessage().getString(),
            error.getDisplayMessage().getString(),
            error
        )));
        FATAL_ERRORS.forEach((mod, errors) -> errors.forEach(error -> log.fatal(
            "Client fatal error entry [{}] title='{}' message='{}'",
            mod.getModId(),
            error.getTitleMessage().getString(),
            error.getDisplayMessage().getString(),
            error
