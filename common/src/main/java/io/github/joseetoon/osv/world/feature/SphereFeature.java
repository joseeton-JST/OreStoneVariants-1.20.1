...
public class SphereFeature extends Feature<SphereConfig> {
    ...
    public boolean place(FeaturePlaceContext<SphereConfig> ctx) {
        final Random rand = new Random(ctx.random().nextLong());
        final double radX = ctx.config().radiusX.rand(rand);
        final double radY = ctx.config().radiusY.rand(rand);
        final double radZ = ctx.config().radiusZ.rand(rand);
